...
//  pp.get("refine_thresh",   a_params.m_refineThresh);
  pp.query("grad_density_refine_thresh",a_params.m_gradDenseRefineThresh);
  pp.query("grad_energy_refine_thresh",a_params.m_gradEnergyRefineThresh);
  pp.query("vorticity_refine_thresh",a_params.m_vorticityRefineThresh);
      ...
      if(a_iprob == 2)
      ...
      else
        {
          ...
          if(a_iprob == 1)
            ...
            {
              pout() << "inflow outflow BCs" << endl;
              InflowOutflowParams ibc_params;
              ParseInflowOutflowParams(ibc_params);

              a_params.m_doBCVelo = RefCountedPtr<BaseDomainBCFactory>(new InflowOutflowVelocityDomBCFactory(ibc_params));

              a_params.m_doBCTemp = RefCountedPtr<BaseDomainBCFactory>(new InflowOutflowTemperatureDomBCFactory(ibc_params));
            }
          else
            {
...
//  ppgodunov.get ("refine_thresh", a_params.m_refineThresh);
  ...
  ppgodunov.query("grad_density_refine_thresh",a_params.m_gradDenseRefineThresh); 
  ppgodunov.query("grad_energy_refine_thresh",a_params.m_gradEnergyRefineThresh);
  ppgodunov.query("vorticity_refine_thresh",a_params.m_vorticityRefineThresh);
  ...
  // SFD params
  ppgodunov.get("do_SFD", a_params.m_doSFD);
  if (a_params.m_doSFD)
    {
      ppgodunov.get("restart_SFD", a_params.m_restartSFD);
      ppgodunov.get("plot_SFD", a_params.m_plotSFD);
      ppgodunov.get("nFilters", a_params.m_nFilters);
      ppgodunov.get("smallest_filter", a_params.m_smallestFilter);
      if (a_params.m_nFilters == 1) 
        {
          a_params.m_largestFilter = a_params.m_smallestFilter;
        }
      else
        {
          ppgodunov.get("largest_filter", a_params.m_largestFilter);
        }
      ppgodunov.get("control_coef", a_params.m_controlCoef);
    }
...
AMR setupProblem (const Box& a_domain, const RealVect& a_dx)
{
  EBAMRCNSParams params;

  // parmparse iprob from input file in next step...
  int iprob =3; // 0=inflow-outflow. 1=solid walls  
  ...
  ParseInflowOutflowParams(ibc_params);
  InflowOutflowIBCFactory bcfactory(ibc_params);
  ...
  EBAMRCNSFactory amrg_fact(params, patchGamma);

  AMR retval;
  setupAMR(retval, params, amrg_fact, a_domain, false, -1.0);
  return retval;
}
