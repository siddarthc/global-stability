...
convergedToSteadyState()
{
  ...
  if (!m_params.m_doSFD)
    ...
    {
      return m_SFDOp.convergedToSteadyState(m_stateNew, m_eblg);
    }
}
//---------------------------------------------------------------------------------------
EBAMRCNS::
       ...
       int                  a_ref_ratio)
{
  ...
  // SFD:
  if (m_params.m_doSFD) m_SFDOp.define(m_params.m_nFilters, m_params.m_smallestFilter, m_params.m_largestFilter, m_params.m_controlCoef, m_nComp);
}
//---------------------------------------------------------------------------------------
/*
void EBAMRCNS::
...
finalAdvance(LevelData<EBCellFAB>& a_Ustar)
{
  ...
  if (m_params.m_doSFD)
    {
      pout() << "doing SFD step for level = " << m_level << endl;
      m_SFDOp(a_Ustar, m_dt, m_eblg); 
    } 
  ...
  if (m_hasFiner)
    {
      ...
      if (m_params.m_doSFD) m_SFDOp.doEBCoarseAverage(&(finePtr->m_SFDOp), finePtr->m_ebCoarseAverage);
    }
...
//---------------------------------------------------------------------------------------
void EBAMRCNS::
getCoarserVelocity(LevelData<EBCellFAB>* & a_veloCoar) const
{
  CH_TIME("getCoarserVelocity");
  ...
  if(m_hasCoarser)
    {
      EBAMRCNS* coarPtr = getCoarserLevel();
      EBCellFactory factCoar(coarPtr->m_eblg.getEBISL());
      veloCoar = new LevelData<EBCellFAB>(coarPtr->m_eblg.getDBL(), SpaceDim, m_nGhost*IntVect::Unit, factCoar);

      Real tCoarNew = coarPtr->m_time;
      Real tCoarOld = tCoarNew - coarPtr->m_dt;

      if (((tCoarNew - tCoarOld) > 1.0e-12) && (m_time >= tCoarOld) && (m_time <= tCoarNew))
        {

          LevelData<EBCellFAB>    veloCoarOld(coarPtr->m_eblg.getDBL(), SpaceDim, m_nGhost*IntVect::Unit, factCoar);
          LevelData<EBCellFAB>    veloCoarNew(coarPtr->m_eblg.getDBL(), SpaceDim, m_nGhost*IntVect::Unit, factCoar);
        ...
        { 
          coarPtr->getVelocity(*veloCoar, coarPtr->m_stateOld); // not safe!
        } 
...
//---------------------------------------------------------------------------------------
void EBAMRCNS::
computeVorticity(LevelData<EBCellFAB>& a_vort,
                 const LevelData<EBCellFAB>& a_state) const
{
  CH_TIME("EBAMRCNS::comouteVorticity");
  ...
  a_vort.define(m_eblg.getDBL(), ncomp, IntVect::Zero, fact);

  LevelData<EBCellFAB> velo(m_eblg.getDBL(), SpaceDim, IntVect::Unit, fact);
  LevelData<EBCellFAB>* veloCoar = NULL;

  getVelocity(velo, a_state);


  if (m_hasCoarser)
    {    

      getCoarserVelocity(veloCoar);
      Interval interv(0,SpaceDim-1);
      int nvarQuad = SpaceDim;
      int nRefCrse = m_coarser_level_ptr->refRatio();
      EBAMRCNS* coarPtr = getCoarserLevel();
      const EBLevelGrid& coEBLG = coarPtr->m_eblg;
      RefCountedPtr<EBQuadCFInterp> quadCFI = RefCountedPtr<EBQuadCFInterp>
      ...
      quadCFI->interpolate(velo, *veloCoar, interv);
      delete veloCoar;
...
/*
      ...
      EBPWLFillPatch patcher(m_eblg.getDBL()  , coarCNS->m_eblg.getDBL(),
                             m_eblg.getEBISL(), coarCNS->m_eblg.getEBISL(),
                             coarCNS->m_eblg.getDomain(), m_ref_ratio, SpaceDim, nghost);
      EBCellFactory coarFact(coarCNS->m_eblg.getEBISL());
      LevelData<EBCellFAB> veloCoarNew(coarCNS->m_eblg.getDBL(), SpaceDim, nghost*IntVect::Unit, coarFact);
      LevelData<EBCellFAB> veloCoarOld(coarCNS->m_eblg.getDBL(), SpaceDim, nghost*IntVect::Unit, coarFact);
      coarCNS->getVelocity(veloCoarNew, coarCNS->m_stateNew);
      coarCNS->getVelocity(veloCoarOld, coarCNS->m_stateOld);

      Real coarTimeOld = 0.0;
      Real coarTimeNew = 1.0;
      Real fineTime = 0.0;
      patcher.interpolate(velo, veloCoarOld, veloCoarNew, coarTimeOld, coarTimeNew, fineTime, Interval(0,SpaceDim-1));
*/
    }

  velo.exchange(Interval(0, SpaceDim-1));

  const DisjointBoxLayout& grid = m_eblg.getDBL();
  const EBISLayout&  ebis = m_eblg.getEBISL();
  ...
  for(DataIterator dit = grid.dataIterator(); dit.ok(); ++dit)
    {
      ...
      const EBCellFAB&  veloFAB = velo[dit()];
...
//  EBLevelDataOps::setCoveredVal(a_vort, 0.0);
}

//---------------------------------------------------------------------------------------
void EBAMRCNS::
computeRelativeGrad(LevelData<EBCellFAB>& a_gradMag,
                    const LevelData<EBCellFAB>& a_state,
  ...
  for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
    {
      ...
      const EBCellFAB& stateFab = a_state[dit()];
      ...
      for (int idir = 0; idir < SpaceDim; ++idir)
        {
          ...
          FORT_GETRELATIVEGRAD(CHF_FRA1(regGradFab,idir),
                               CHF_CONST_FRA1(regStateFab,a_index),
                               CHF_CONST_INT(idir),
                               CHF_BOX(bLo),
                               CHF_CONST_INT(hasLo),
                               CHF_BOX(bHi),
          ...
          for(VoFIterator vofit(ivsIrreg, ebisBox.getEBGraph());
              vofit.ok(); ++vofit)
            {
              ...
              Real valCent = stateFab(vof, a_index);
              Real dpl = 0.;
              Real dpr = 0.;
              Real dpc = 0.;
                  ...
                  for(int iface = 0; iface <facesLeft.size(); iface++)
                    {
                      ...
                      valLeft += stateFab(vofLeft, a_index);
                    }
                  ...
                  for(int iface = 0; iface <facesRigh.size(); iface++)
                    {
                      ...
                      valRigh += stateFab(vofRigh, a_index);
                    }
      ...
      EBCellFAB& gradMagFab = a_gradMag[dit()];
...
tagCells(IntVectSet& a_tags)
{
  ...
  bool tagOnDensity = true;
  pp.query("tag_on_density", tagOnDensity);

  bool tagOnVorticity = false;
  pp.query("tag_on_vorticity", tagOnVorticity);

  int densityIndex = CRHO;
  int energyIndex = CENG;

  Interval intervDensity(densityIndex, densityIndex);
  Interval intervDensMom(CRHO, CMOMX+SpaceDim-1);
  Interval intervEnergy(energyIndex,energyIndex);
...
// begin tag on density
  if (tagOnDensity)
    {
      LevelData<EBCellFAB> magGrad(m_eblg.getDBL(), 1, IntVect::Unit, factory);
      if (m_hasCoarser)
         {
           const EBAMRCNS* coarCNS = getCoarserLevel();
      ...
      computeRelativeGrad(magGrad, consTemp, densityIndex);

      for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
        {
          const Box& b = m_eblg.getDBL().get(dit());
          const EBISBox& ebisBox = m_eblg.getEBISL()[dit()];
          const EBCellFAB& gradMagFab = magGrad[dit()];
          // Tag where value exceeds threshold
          ...
          for(VoFIterator vofit(ivsTot, ebisBox.getEBGraph());
              vofit.ok(); ++vofit)
            {
              ...
              if ((iv[0] > imin) && (gradMagFab(vof, 0) >= m_params.m_gradDenseRefineThresh))
                {
                  localTags |= iv;
                }
            }
  ...
  if (tagOnEnergy)
    {
      ...
      consTemp.exchange(intervEnergy);
      computeRelativeGrad(magGrad, consTemp, energyIndex);
      ...
      for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
        {
          ...
          for(VoFIterator vofit(ivsTot, ebisBox.getEBGraph());
              vofit.ok(); ++vofit)
            {
              ...
              if ((iv[0] > imin) && (gradMagFab(vof, 0) >= m_params.m_gradEnergyRefineThresh))
                {
                  localTags |= iv;
                }
            }
...
*/
      LevelData<EBCellFAB> vorticity;
//      computeVorticity(vorticity, consTemp);
      computeVorticity(vorticity, m_stateNew);
      int nVortComp = vorticity.nComp();

      for (DataIterator dit = m_eblg.getDBL().dataIterator(); dit.ok(); ++dit)
        {
          const Box& b = m_eblg.getDBL().get(dit());
          const EBISBox& ebisBox = m_eblg.getEBISL()[dit()];
          const EBCellFAB& vortFab = vorticity[dit()];
          // Tag where value exceeds threshold
          ...
          for(VoFIterator vofit(ivsTot, ebisBox.getEBGraph());
              vofit.ok(); ++vofit)
            {
              ...
              Real vortMag = 0.0;
              for (int icomp = 0; icomp < nVortComp; icomp++)
                {
              ...
              if (vortMag >= m_params.m_vorticityRefineThresh)
                {
                  localTags |= iv;
                }
            }
  ...
  if (m_params.m_doSFD) 
    {  
      if (m_hasCoarser)
        {
          EBAMRCNS* coarPtr = getCoarserLevel();
          m_SFDOp.regrid(stateSaved, &(coarPtr->m_SFDOp), m_eblg, m_ebFineInterp);
        }
      else
        {
          m_SFDOp.regrid(stateSaved, NULL, m_eblg, m_ebFineInterp); 
        }
    }
}
//-------------------------------------------------------------------------
...
initialData()
{
  ...
  //SFD:
  if (m_params.m_doSFD) m_SFDOp.initialize(m_stateNew, m_eblg); 
  ...
  if (m_hasFiner)
    {
      ...
      if (m_params.m_doSFD) m_SFDOp.postInitialize(&(finePtr->m_SFDOp), finePtr->m_ebCoarseAverage);
    }
...
writeCheckpointLevel(HDF5Handle& a_handle) const
{
  ...
  if (m_params.m_doSFD)
    {
      m_SFDOp.writeCheckpointLevel(a_handle);
    }
...
readCheckpointLevel(HDF5Handle& a_handle)
{
  ...
  if (m_params.m_doSFD)
    {
      if (m_params.m_restartSFD)
        {
          m_SFDOp.readCheckpointLevel(a_handle, m_eblg);
        }
      else
        {
          m_SFDOp.initialize(m_stateNew, m_eblg);
        }
    }
}
//-------------------------------------------------------------------------
...
writePlotHeaderOld(HDF5Handle& a_handle) const
{
  ...
  bool pltVort;
  ParmParse pp;
  pp.query("plot_vorticity", pltVort);
  int nVort = pltVort ? 1 : 0;
  if ((SpaceDim == 3) && pltVort) nVort = 3;

  int indexVort = consAndPrim;
  int indexVolFrac = consAndPrim + nVort;
  ...
  CH_assert(nCompTotal == consAndPrim + nVort + 3*SpaceDim+2);
...
*/
  if (pltVort)
    {
      Vector<string> vortNames;
      string vortComp("vorticity");
      if (SpaceDim == 2)
        {
          vortNames.resize(1);
          vortNames[0] = "z-" + vortComp;
        }
      else
        {  
          vortNames.resize(3);
          vortNames[0] = "x-" + vortComp;
          vortNames[1] = "y-" + vortComp;
          vortNames[2] = "z-" + vortComp;
        }
        
        for (int i = 0; i < nVort; i++)
          {
            names[indexVort + i] = vortNames[i];
          }
    }
  ...
  if (m_params.m_doSFD && m_params.m_plotSFD) m_SFDOp.writePlotHeader(header, nCompTotal);
...
void EBAMRCNS::writePlotLevelOld(HDF5Handle& a_handle) const
{
  ...
  Vector<Real> coveredValuesVort(nVort, coveredVal);

  Vector<Real> coveredValues;
  coveredValues.append(coveredValuesCons);
  coveredValues.append(coveredValuesPrim);
  coveredValues.append(coveredValuesVort);

  if (m_params.m_doSFD && m_params.m_plotSFD) nCompTotal += m_SFDOp.getnComp();
...
//      ParmParse pp;
      ...
      currentFab.copy(vortfab.getSingleValuedFAB(),0,indexVort, nVort);
  ...
  if (m_params.m_doSFD && m_params.m_plotSFD) m_SFDOp.appendDataToFAB(fabData, indexDist+1, m_eblg);
