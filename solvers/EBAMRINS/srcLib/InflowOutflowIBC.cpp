  ...
  for (int idir = 0; idir < SpaceDim; idir++)
    {
      if (idir != m_flowDir)
        {
          EBLevelDataOps::setVal(a_velocity, 0.0, idir);
        }
    }
  ...
  if (m_params.m_doInitWhiteNoise)
    {
      ...
      for (DataIterator dit = a_velocity.dataIterator(); dit.ok(); ++dit)
        {
          DataIndex d = dit();
          EBCellFAB& velState = a_velocity[d];
          BaseFab<Real>& regVelo = velState.getSingleValuedFAB();
          const EBISBox& ebisBox = velState.getEBISBox();
          const Box& region = velState.getRegion();
