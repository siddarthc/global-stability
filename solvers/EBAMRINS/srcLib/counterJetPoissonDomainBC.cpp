...
bool s_higherOrderVelBC = false;

//pressure stencil
void counterJetPoissonDomainBC::getFluxStencil(      VoFStencil&      a_stencil,
...
// start hardwire
      ...
      tubeRadius += wallThickness * bcFactor;
// end hardwire
...
//         a_faceFlux = EBArith::extrapFaceVelToOutflow(a_face,a_side,a_idir,ebisBox.getEBGraph(),a_vel[a_idir],0);//be careful of this 0, it is the component 
         a_faceFlux = 0.;
         if (velcomp == m_flowDir) a_faceFlux = m_jet1inflowVel;
       }
     else
       {
...
void counterJetPoissonDomainBC::getFaceFlux(BaseFab<Real>&        a_faceFlux,
...
//////called by MAC projection solver for domain bc on phi
bool counterJetPoissonDomainBC::
...
void counterJetPoissonDomainBC::getFaceFlux(Real&                 a_faceFlux,
                                               const VolIndex&       a_vof,
                                               const int&            a_comp,
...
//called by projection solve for irreg x domain
void counterJetPoissonDomainBC::getInhomFaceFlux(Real&                 a_faceFlux,
                                                    const VolIndex&       a_vof,
                                                    const int&            a_comp,
                                                    const EBCellFAB&      a_phi,
                                                    const RealVect&       a_probLo,
...
void counterJetPoissonDomainBC::getFaceGradPhi(Real&                 a_faceFlux,
                                                  const FaceIndex&      a_face,
                                                  const int&            a_comp,
                                                  const EBCellFAB&      a_phi,
                                                  const RealVect&       a_probLo,
...
//velocity stencil
void counterJetHelmholtzDomainBC::getFluxStencil(      VoFStencil&      a_stencil,
                                                    const VolIndex&        a_vof,
                                                    const int&             a_comp,
                                                    const RealVect&        a_dx,
                                                    const int&             a_idir,
...
///
/**
   This never gets called.  counterJetPoissonDomainBC::getFaceVel takes care of it.
*/
void
...
getFaceVel(Real&                 a_faceFlux,
           ...
           const bool&           a_doDivFreeOutflow)
{
  MayDay::Error("counterJetHelmholtzDomainBC::getFaceVel is not needed for Helmholtz.");
}
///
/**
...
*/
void counterJetHelmholtzDomainBC::getFaceFlux(BaseFab<Real>&        a_faceFlux,
                                                 const BaseFab<Real>&  a_phi,
                                                 const RealVect&       a_probLo,
                                                 const RealVect&       a_dx,
                                                 const int&            a_idir,
...
//  bool isVelNeum =  (isOutflow || isSlipWall);
  ...
  if (!m_adjointSolver)
  {
    ...
    else if (isInflow || isFreeStreamBndry)
    {
      DirichletPoissonDomainBC diriBC;
...
void counterJetHelmholtzDomainBC::getFaceFlux(Real&                 a_faceFlux,
                                                 const VolIndex&       a_vof,
                                                 const int&            a_comp,
                                                 const EBCellFAB&      a_phi,
                                                 const RealVect&       a_probLo,
...
bool counterJetHelmholtzDomainBC::
isDirichletDom(const VolIndex&   a_ivof,
               const VolIndex&   a_jvof,
               const EBCellFAB&  a_phi) const
{
...
*/
  MayDay::Error("counterJetPoisonDomainBC::isDirichletDom not set");
  return true; // to shut the compiler
}

//called by EBAMRPoissonOp::applyOp for viscous Helmholtz EB x domain
void counterJetHelmholtzDomainBC::getInhomFaceFlux(Real&                 a_faceFlux,
                                                      const VolIndex&       a_vof,
                                                      const int&            a_comp,
                                                      const EBCellFAB&      a_phi,
                                                      const RealVect&       a_probLo,
...
void counterJetHelmholtzDomainBC::getFaceGradPhi(Real&                 a_faceFlux,
                                                    const FaceIndex&      a_face,
                                                    const int&            a_comp,
                                                    const EBCellFAB&      a_phi,
                                                    const RealVect&       a_probLo,
                                                    ...
                                                    const bool&           a_useHomogeneous)
{
  MayDay::Error("counterJetHelmholtzDomainBC::getFaceGradPhi not needed");
}
