...
{
  if (m_params.m_doSFD) initializeSFDData();
}
/*********************/
void
EBAMRNoSubcycle::initializeSFDData()
{
  for (int ilev = 0; ilev <= m_finestLevel; ilev++)
    {
      m_SFDOp[ilev].initialize(*m_velo[ilev], m_eblg[ilev]);
    }
}
/*********************/
void
  ...
  for (int ilev=1; ilev<= m_finestLevel; ilev++)
    {
      ...
      if (m_params.m_doSFD) m_SFDOp[ilev].regrid(*tempLDPtr[ilev],&m_SFDOp[ilev-1],m_eblg[ilev], ebInterpVec);
...
/*****************/
void EBAMRNoSubcycle::averageDownExtraTerms()
{
  if (m_params.m_doSFD && !m_advanceGphiOnly)
    {
      averageDownSFDData();
...
/*****************/
void EBAMRNoSubcycle::averageDownSFDData()
{
  for (int ilev = m_finestLevel; ilev > 0; ilev--)
    {
      for (int iFilter = 0; iFilter < m_params.m_nFilters; iFilter++)
        {
          int ncomp = (*(m_SFDOp[ilev].getData()))[iFilter]->nComp();
          Interval interval(0, ncomp-1);
          RefCountedPtr<EBCoarseAverage> avePtr = m_aveOper[ilev];
          if (ncomp == SpaceDim)
            {
          ...
          ebaverage.average(*((*(m_SFDOp[ilev - 1].getData()))[iFilter]),
                            *((*(m_SFDOp[ilev].getData()))[iFilter]),
                            interval);
      ...
      for (int iFilter = 0; iFilter < m_params.m_nFilters; iFilter++)
        {
          int ncomp = (*(m_SFDOp[ilev].getData()))[iFilter]->nComp();
          Interval interval(0, ncomp-1);
          (*(m_SFDOp[ilev].getData()))[iFilter]->exchange(interval);
        }
    }
}
/*****************/
void EBAMRNoSubcycle::
defineExtraTerms(int a_startLevel)
{
  if (m_params.m_doSFD) defineSFDOp(a_startLevel);
}
/*****************/
void
EBAMRNoSubcycle::
defineSFDOp(int a_startLevel)
{
  int nlevels = m_params.m_maxLevel + 1;
  m_SFDOp.resize(nlevels);

  if (m_params.m_verbosity > 3)
    {
      pout() << "EBAMRNoSubcycle::defineSFDOp" << endl;
    }
  int startLevel = 0;
  if (a_startLevel > startLevel)
    {
...
addExtraOutputData(LevelData<EBCellFAB>& a_outputData, const int a_startDstInterv, const int a_ilev)
{
  if (m_params.m_doSFD)
    {
      int startDstInterv = a_startDstInterv;
      for (int iFilter = 0; iFilter < m_params.m_nFilters; iFilter++)
        {
          int nComp = (*(m_SFDOp[a_ilev].getData()))[iFilter]->nComp();
          Interval srcInterv(0 , nComp-1);
          Interval dstInterv(startDstInterv, startDstInterv+nComp-1);
          (*(m_SFDOp[a_ilev].getData()))[iFilter]->copyTo(srcInterv, a_outputData, dstInterv);
          startDstInterv += nComp;
        }
    }
}
/*****************/
...
/*****************/
void
EBAMRNoSubcycle::writeExtraDataToCheckpoint(HDF5Handle& a_handleOut, const int a_ilev)
{
  if (m_params.m_doSFD) m_SFDOp[a_ilev].writeCheckpointLevel(a_handleOut);
}
/*****************/
void
...
void
EBAMRNoSubcycle::
readExtraDataFromCheckpoint(HDF5Handle& a_handleIn, const int a_ilev)
{
  if (m_params.m_doSFD)
    {
      if (m_params.m_restartSFD)
        {
          m_SFDOp[a_ilev].readCheckpointLevel(a_handleIn, m_eblg[a_ilev]);
        }
/*
      else
        {
          m_SFDOp[a_ilev].initialize(*m_velo[a_ilev], m_eblg[a_ilev]);
        }
*/
    }
}
  ...
  if (oldFinestLevel < finestLevelFromParmParse)
    {
      ...
      Vector<Vector<LevelData<EBCellFAB>* > > tempSFDLDPtr;
      if (m_params.m_doSFD && m_params.m_restartSFD) tempSFDLDPtr.resize(oldFinestLevel+1);
      ...
      for (int ilev=0; ilev<= oldFinestLevel; ilev++)
        {
          ...
          if (m_params.m_doSFD && m_params.m_restartSFD)
            {
              tempSFDLDPtr[ilev].resize(m_params.m_nFilters);
              for (int iFilter = 0; iFilter < m_params.m_nFilters; iFilter++)
                {
                  (tempSFDLDPtr[ilev])[iFilter] = new LevelData<EBCellFAB>();
                  (tempSFDLDPtr[ilev])[iFilter]->define(m_grids[ilev], m_velo[ilev]->nComp(), 3*IntVect::Unit, ebcellfact);
                  Interval SFDInterv(0, m_velo[ilev]->nComp()-1);
                  (*(m_SFDOp[ilev].getData()))[iFilter]->copyTo(SFDInterv, *((tempSFDLDPtr[ilev])[iFilter]), SFDInterv);
                }
            }
        }
          ...
          if (m_params.m_doSFD && m_params.m_restartSFD)
            {
              for (int iFilter = 0; iFilter < m_params.m_nFilters; iFilter++)
                {
                  Interval SFDInterv(0, m_velo[ilev]->nComp()-1);
                  (tempSFDLDPtr[ilev])[iFilter]->copyTo(SFDInterv, *((*(m_SFDOp[ilev].getData()))[iFilter]), SFDInterv);
                }
            }
        }
          ...
          if (m_params.m_doSFD && m_params.m_restartSFD)
            {
              ...
              for (int iFilter = 0; iFilter < m_params.m_nFilters; iFilter++)
                {
                  ebInterpVec.interpolate(*((*(m_SFDOp[ilev].getData()))[iFilter]), *((*(m_SFDOp[ilev-1].getData()))[iFilter]), SFDInterv);
                }
            }
        }
      ...
      // delete temp data
      for (int ilev=0; ilev < tempLDPtr.size(); ilev++)
        {
          delete tempLDPtr[ilev];
          delete tempLDPtr2[ilev];
          if (m_params.m_doSFD && m_params.m_restartSFD)
            {
              for (int iFilter = 0; iFilter < m_params.m_nFilters; iFilter++)
                {
                  delete (tempSFDLDPtr[ilev][iFilter]);
                }
            }
        }
...
// Sid added 10/13/16: There is an issue with m_advVel's EBBox when restarted with different maxLevel. This is a quick fix:

      for (int ilev = 0; ilev <=m_finestLevel; ilev++)
        {
          EBFluxFactory ebfluxfact(m_ebisl[ilev]);
          m_advVel[ilev]->define(m_grids[ilev], 1, 3*IntVect::Unit, ebfluxfact);
          EBLevelDataOps::setToZero(*m_advVel[ilev]);
        }
    }
